...
description: Wait for CI to settle across all repos in a private Polygraph session; for same-installation shared session IDs, report read-only metadata and that CI/log details are unavailable. USE WHEN user says "await polygraph", "wait for polygraph ci", "polygraph ci status", "check polygraph ci", "watch polygraph session", "monitor polygraph".
{% if platform == "claude" %}
user-invocable: true
allowed-tools:
  - Bash
