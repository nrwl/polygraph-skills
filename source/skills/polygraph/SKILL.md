...
| --- | --- | --- |
| `list_repos` | `polygraph repo list` | Discover candidate repositories. Hard filters (`connectedTo`+`connectionType`, packages, APIs, names) narrow the pool — all specified filters must hold, package/API/name matching is case-insensitive substring (e.g. "axios" also matches "axios-retry"). At most one of `semanticQuery`/`similarToRepo` ranks the pool by similarity; ranking never adds or removes repos, and without it results are ordered by recent activity. Each result carries `signals` evidence (graph distance/direction/via, similarity, matched values). |
