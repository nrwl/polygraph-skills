...
| --- | --- | --- |
| `list_repos` | `polygraph repo list` | Discover candidate repositories. Optional parameters: `connectedTo` (+ `connectionType`), `publishedPackages`, `consumedPackages`, `publishedApis`, `consumedApis`, `nameFilter`, `semanticQuery`, `similarToRepo`, `limit`. Returns `repos` (each with `signals` evidence), plus `total`, `ranked`, and `notice`. |
